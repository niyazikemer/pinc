...
pinc separates the idea of a tool from any specific frontend:

- `pinc_agent` defines provider-neutral tool types and executes requested tool calls.
- `pinc_coding` provides concrete local coding tools for files and shell commands.
...
`pinc_coding` provides four built-in local coding tools:
...
```python
from pinc_coding import create_coding_tools
...
Relative paths passed to the tools are resolved against `cwd`. If `cwd` is omitted, pinc uses the process current working directory at the time the factory is called.
...
For matching, pinc normalizes file content and edit text to LF line endings. After applying edits, pinc restores the file's original dominant line ending. UTF-8 byte-order marks are preserved.
...
On POSIX systems, commands are started in a new session. If a timeout occurs, pinc kills the whole process group so child processes from pipelines or compound commands are stopped too. On non-POSIX systems, pinc kills the direct subprocess.
...
When truncation happens, pinc writes the full command output to a temporary `.log` file and includes that path in the result metadata.
